...
2.  **Function Call:** The `model.py` script retrieves these parameters from the session state and passes them to the appropriate backend function, typically from `des_parallel_process.py` ([Simulation Runner & Parallelization](07_simulation_runner___parallelization__.qmd)).
...
5.  **Data Processing & Display:** The `model.py` script then uses functions (often from `visualisation` scripts or covered in [Results Processing & Visualization](08_results_processing___visualization__.qmd)) to read these CSV files, calculate summary statistics, and create plots.
